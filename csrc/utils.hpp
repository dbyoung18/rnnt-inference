...
long get_duration (const mlperf::PerfClock::time_point start, const mlperf::PerfClock::time_point end);
...
public:
  ProfileRecord (
      bool is_record, const std::string& profiler_file) : is_record_(is_record), profiler_file_(profiler_file) {
    if (is_record_)
